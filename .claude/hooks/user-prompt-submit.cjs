#!/usr/bin/env node
// Claude UserPromptSubmit Hook - registry-driven skill activation.

const fs = require('fs');
const path = require('path');
const { recordHookDecision, recordHookMetric } = require('../../.codex/hooks/hook-metrics.cjs');
const {
  ...
  skipPatterns,
  traitsFromMatches
} = require('../../.codex/hooks/hook-routing.cjs');
const {
  matchedRegistryTermsForSkills,
  maxSelectedSkills,
  selectFallbackSkills,
  selectRegistrySkills
} = require('../../.codex/hooks/hook-skill-selection.cjs');
const {
...
} = require('../../.codex/hooks/hook-output.cjs');
...
if (interactionMode === 'snapshot') {
  writeAdditionalContext(formatBudgetContext(interactionMode, interactionPolicy, projectRoot, promptTraits).join('\n'));
}
...
} else if (interactionMode !== 'chat' && interactionMode !== 'snapshot') {
...
const sections = [];
sections.push(...formatBudgetContext(interactionMode, interactionPolicy, projectRoot, promptTraits), '');
...
function writeAdditionalContext(additionalContext) {
  finish(additionalContext, { mode: interactionMode, selectedSkills: selectedMetricSkills });
}
